...
# Choose micro / macro predictor variables
...
df <- df_all %>%
  mutate(
    iso = if ("ISO_Code" %in% names(df_all)) ISO_Code else NA,
    country = if ("Countries" %in% names(df_all)) Countries else NA
  ) %>%
...
if (tolower(METHOD) == "mcmc" && "EFS_pred" %in% names(draws)) {
  ...
  # Ensure N and ndraws are numeric
N <- as.numeric(nrow(df_model))  # number of observations

if (tolower(METHOD) == "mcmc") {
  ndraws <- length(draws$phi)
} else {
  ndraws <- nrow(as.matrix(draws_mat))
}

# Safety fallback
if (is.na(ndraws) || ndraws == 0) {
  stop("Error: ndraws could not be determined. Check your Stan sampling output.")
}
  # Preallocate
...
for (iso_code in sample_isos) {
  ...
  plot(sub_hist$year, sub_hist$EFS, type = 'b', pch = 19, col = 'black',
       ylab = 'EFS', xlab = 'Year',
       main = paste0(sub_hist$country[1], " (", iso_code, ") - observed vs posterior mean"))
  points(sub_hist$year, df_out$EFS_pred_mean[idxs], col = 'blue', pch = 18)
  arrows(sub_hist$year, df_out$EFS_pred_lo[idxs], sub_hist$year, df_out$EFS_pred_hi[idxs],
